...
from .base import BaseArgParser, ArgFlag, ArgParseError
from ..collection import to_list
...
class ArgParser(BaseArgParser):
    ...
    def _parse_double_minus(self, arg_name, shift):
        ...
        if ind == len(arg_name) - 1:
            if argument.flag == ArgFlag.Boolean:
                self._key_value_map[key] = True
                return
            raise ArgParseError()
    ...
    def _init_name_key_map(self):
        for (k, v) in self._key_argument_map.items():
            for n in v.names:
    ...
    def _init_attr_dict(self):
        ...
        for k, v in self._key_value_map.items():
            # self.__dict__[k] = v
            setattr(self, k, v)
    ...
    def _add_all(self, key, argument, value):
        ...
        if argument.flag == ArgFlag.String:
            self._key_value_map[key] = value
            return

        if argument.flag == ArgFlag.List:
    ...
    def _add_all_forward(self, key, argument, shift):
        ...
        if argument.flag == ArgFlag.String:
            if not len(shift):
                raise ArgParseError()
            self._key_value_map[key] = shift.pop()
            return
    ...
    def _add_bool(self, key, value):
        ...
        if value in ["true", "t", "yes", "y", "1"]:
            ...
            self._key_value_map[key] = False
        else:
            raise ArgParseError()

    def _add_list(self, key, value):
        if key in self._key_value_map:
            array = self._key_value_map[key]
            array.append(value)
        else:
            self._key_value_map[key] = [value]
    ...
    def _add_property(self, key, hkey, hval):
        if key in self._key_value_map:
            m = self._key_value_map[key]
            m[hkey] = hval
        else:
            self._key_value_map[key] = {hkey: hval}
    ...
    def _get_key_or_raise(self, name):
        ...
        return key, self._key_argument_map[key]


def _assert_not_property(argument):
    if argument.flag == ArgFlag.Property:
        raise ArgParseError()
